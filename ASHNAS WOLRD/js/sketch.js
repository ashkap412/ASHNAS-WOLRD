...
function setup() {
  ...
  rad = height/9; // compute radius for central circle
  background(400); // clear the screen

  for (var i = 0; i<sines.length; i++) {
    sines[i] = PI; // start EVERYBODY facing SOUTH
  }
}

function draw() {
  if (!trace) {
    background(300); // clear screen if showing geometry
    stroke(3, 255); // black pen
    noFill(); // don't fill
  } 
  ...
  for (var i = 0; i<sines.length; i++) {
    ...
    var radius = rad/(i+1); // radius for circle
    rotate(sines[i]); // rotate circle
    if (!trace) ellipse(0, 0, radius*8, radius*7); // if we're simulating, draw the sine
    push(); // go up one level
    translate(0, radius); // move to sine edge
    if (!trace) ellipse(0, 0, 5, 5); // draw a square
    if (trace) ellipse(0, 0, erad, erad); // draw with erad if tracing
    pop(); // go down one level
    translate(0, radius); // move into position for next cossine
